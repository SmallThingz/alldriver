...
pub fn main() !void {
    ...
    for (installs, 0..) |install, index| {
        ...
        std.debug.print("Launch browser host: {s} ({s}) from {s}\n", .{
        ...
        var browser_host = driver.launch(allocator, .{
            .install = install,
            ...
            last_error = err;
            std.debug.print("Host launch failed: {s}\n", .{@errorName(err)});
            continue;
        };
        defer browser_host.deinit();

        if (browser_host.endpoint) |ep| {
            std.debug.print("Host endpoint: {s}\n", .{ep});
        }
        waitForEndpointReachable(browser_host.endpoint, endpoint_connect_timeout_ms) catch |err| {
            last_error = err;
            std.debug.print("Host endpoint unreachable: {s}\n", .{@errorName(err)});
            continue;
        };

        const endpoint = browser_host.endpoint orelse {
            last_error = error.InvalidEndpoint;
            std.debug.print("Host endpoint missing.\n", .{});
            continue;
        };

        var session = driver.attachWebView(allocator, .{
            .kind = .android_webview,
            .endpoint = endpoint,
        }) catch |err| {
            last_error = err;
            std.debug.print("attachWebView failed: {s}\n", .{@errorName(err)});
            continue;
        };
        defer session.deinit();

        std.debug.print("Attached mode={s} transport={s}\n", .{
